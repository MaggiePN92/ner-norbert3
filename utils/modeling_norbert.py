...
try:
    ...
    from utils.configuration_norbert import NorbertConfig
...
class LSTM_Classifier(nn.Module):
    def __init__(self, config, num_labels: int):
        ...
        self.class_layer = nn.Linear(config.hidden_size, num_labels)
    ...
    def initialize(self, hidden_size):
        ...
        nn.init.trunc_normal_(self.nonlinearity[1].weight, mean=0.0, std=std, a=-2*std, b=2*std)
        # nn.init.trunc_normal_(self.nonlinearity[-1].weight, mean=0.0, std=std, a=-2*std, b=2*std)
        nn.init.trunc_normal_(self.class_layer.weight, mean=0.0, std=std, a=-2*std, b=2*std)
        self.nonlinearity[1].bias.data.zero_()
        # self.nonlinearity[-1].bias.data.zero_()
        self.class_layer.bias.data.zero_()
    def forward(self, x):
        lstm_output, _ = self.nonlinearity(x)
        x = self.class_layer(lstm_output)
        return x
...
class NorbertELMOForTokenClassification(NorbertModel):
    _keys_to_ignore_on_load_unexpected = ["classifier"]
    _keys_to_ignore_on_load_missing = ["head"]

    def __init__(self, config,tokenizer=None):
        super().__init__(config, add_mlm_layer=False)
        config.hidden_size = 1024 # for ELMO
    ...
    def get_contextualized_embeddings_from_elmo(self):
        pass
    ...
    def forward(
        ...
        #     input_text = self.tokenizer.convert_ids_to_tokens(input_id,skip_special_tokens=False)
        #     inputs_text.append(input_text)
        # sequence_output = self.elmo_model.get_elmo_vectors(inputs_text)
        # sequence_output = sequence_output.astype(np.float32)
        # sequence_output = torch.from_numpy(sequence_output).float().to(self.device)
        # sequence_output = self.get_contextualized_embeddings(input_ids, attention_mask)
        logits = self.head(sequence_output)
...
class NorbertLSTMForTokenClassification(NorbertModel):
    _keys_to_ignore_on_load_unexpected = ["classifier"]
    _keys_to_ignore_on_load_missing = ["head"]
    ...
    def __init__(self, config):
        ...
        self.num_labels = config.num_labels
        self.head = LSTM_Classifier(config, self.num_labels)
